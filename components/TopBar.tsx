            ...
            <div className="w-full max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center gap-4">
                ...
                <div className="flex gap-1 md:gap-2 items-center flex-shrink-0 ml-auto">
                    <div className="relative group hidden sm:block mx-1">
                        <div className="absolute inset-y-0 left-3 flex items-center pointer-events-none text-zinc-400"><Search size={14} /></div>
                        <input
                            type="text"
                            value={searchQuery}
                        ...
                        />
                         {searchQuery && <button onClick={() => setSearchQuery("")} className="absolute inset-y-0 right-2 flex items-center text-zinc-400 hover:text-zinc-600 p-1"><X size={12} /></button>}
                    </div>

                    {!isAuxMode && (
                        <>
                            <button onClick={onUndo} disabled={!canUndo} className={`p-2 rounded-full transition-all ${!canUndo ? 'text-zinc-300 dark:text-zinc-700 opacity-50' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`} title="Undo"><RotateCcw size={18} /></button>
                            <button onClick={onRedo} disabled={!canRedo} className={`p-2 rounded-full transition-all ${!canRedo ? 'text-zinc-300 dark:text-zinc-700 opacity-50' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`} title="Redo"><RotateCw size={18} /></button>
                            <div className="w-px h-5 bg-zinc-300 dark:bg-zinc-800 mx-1 hidden sm:block"></div>
                            {mode !== 'edit' && (
                                <button onClick={() => fileInputRef.current?.click()} className="p-2 rounded-full text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all hidden sm:block" title="Import Handwriting">
                                    <Camera size={18} />
                    ...
                    <button onClick={onCopy} className={`p-2 rounded-full transition-all ${copySuccess ? 'text-green-500 bg-green-50' : 'text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`} title="Copy Content">
                        {copySuccess ? <Check size={18} /> : <Copy size={18} />}
                    </button>
                    <button onClick={onExport} className="p-2 rounded-full text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all" title="Export"><Download size={18} /></button>

                    {mode === 'edit' && (
                        <div className="relative">
                            <button onClick={() => setEditMenuOpen(!editMenuOpen)} className={`p-2 rounded-full transition-all ${isGrammarRunning ? 'bg-indigo-50 text-indigo-600' : 'text-zinc-500 hover:bg-zinc-100'}`} title="Edit Tools">
                            ...
                            {editMenuOpen && (
                                ...
                                <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-xl z-20 py-1 overflow-hidden flex flex-col">
                                     <button onClick={() => { onGrammar(); setEditMenuOpen(false); }} disabled={isGrammarRunning} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-left text-sm font-medium"><Wand2 size={16} className="text-blue-500" />Fix Grammar</button>
                                     <button onClick={() => { onApprove(); setEditMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-left text-sm font-medium"><ThumbsUp size={16} className="text-green-500" />Approve All</button>
                                     <button onClick={() => { onRevert(); setEditMenuOpen(false); }} className="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 text-left text-sm font-medium"><ThumbsDown size={16} className="text-red-500" />Revert All</button>
                                </div>
                                </>
                            )}
                        </div>
                    ...
                    <button onClick={onSettings} className="p-3 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-full transition-all text-zinc-600 dark:text-zinc-400">
                        {user ? (user.picture ? <img src={user.picture} alt="Profile" className="w-5 h-5 rounded-full" /> : <User size={20} className="text-indigo-500" />) : <Settings size={20} />}
                    </button>
                </div>
            </div>
        </header>
